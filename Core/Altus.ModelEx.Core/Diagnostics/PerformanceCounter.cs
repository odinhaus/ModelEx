...
namespace Altus.Core.Diagnostics
{
    ...
    public class PerformanceCounter : IDisposable
    {
        ...
        /// <summary>
        /// Grab the Altus instance name - either the friendly name or product key
        /// </summary>
        static PerformanceCounter()
        {
            if (Context.CurrentContext == null) return;
#if BREAKME
            System.Diagnostics.Debugger.Break();
#endif
            try
            {
                _AppInstance = NodeIdentity.NodeAddress;
            }
            catch
            {
                _AppInstance = "UnknownNode:";
            }
            // assign the instance name or else product key if not found
            try
            {
                _AppInstance += Altus.Core.Configuration.ConfigurationManager.GetAppSetting("Instance");
            }
            catch
            {
                _AppInstance += Dns.GetHostName();
            }
        ...
        public static PerformanceCounter CreateCounterInstance(PerformanceCounter pc)
        {
            ...
            {
                Logger.LogError(ExceptionHelper.CreateException(ExceptionCode.PerfMonCouldNotCreateCounter, pc.Name + " " + ioe.Message));
            }
            return pc;
        }
        ...
        {
            InstanceType xc = InstanceType.WindowsService;

            if (Context.CurrentContext != null && Context.CurrentContext.InstanceType != (InstanceType)0)
            {
                xc = Context.CurrentContext.InstanceType;
            }
            return CounterCategory(cat, xc);
        }
        ...
        /// <param name="cat"></param>
        /// <returns></returns>
        public static string CounterCategory(PerformanceCounterCategories cat, InstanceType execCtx)
        {
            String perfCounterSource = String.Empty;

            switch (execCtx)
            {
                case InstanceType.ASPNetHost:
                case InstanceType.WebClient:
                case InstanceType.WebServer:
                    perfCounterSource = "Web";
                    break;
                case InstanceType.WindowsFormsClient:
                case InstanceType.WPFClient:
                    perfCounterSource = "Desktop";
                    break;
                case InstanceType.WindowsService:
                    perfCounterSource = "Service";
                    break;
                default:
                    Logger.LogError("Current Instance type not found: " + Context.CurrentContext.InstanceType);
                    break;
            }
            ...
            switch (cat)
            {
                case PerformanceCounterCategories.Comm:
                    return "Altus:Communications:" + perfCounterSource;
                case PerformanceCounterCategories.Core:
                    return "Altus:Core:" + perfCounterSource;
                case PerformanceCounterCategories.Data:
                    return "Altus:Data:" + perfCounterSource;
                default:
        ...
        public static String CategoryHelp(String cat)
        {
            ...
            try
            {
                ...
                {
                    case PerformanceCounterCategories.Comm:
                        helpString = "Altus Device and Desktop Communications Counters";
                        break;
                    case PerformanceCounterCategories.Core:
                        helpString = "Altus Core Functionality Counters";
                        break;
                    case PerformanceCounterCategories.Data:
                        helpString = "Altus Data Access Counters";
                        break;
                }
            }
            catch
            {
                helpString = "Altus General Counters";
            }
            return helpString;
        }
        ...
        public static PerformanceCounterInstallAttribute[] LoadPerfmonInstallerAttributes()
        {
            ...
            if (Context.CurrentContext.InstanceType.HasFlag(InstanceType.WindowsFormsClient)
                || Context.CurrentContext.InstanceType.HasFlag(InstanceType.WPFClient)
                || Context.CurrentContext.InstanceType.HasFlag(InstanceType.WindowsService))
            {
            ...
            else if (Context.CurrentContext.InstanceType.HasFlag(InstanceType.Other))
            {
                //    asmDir = Context.CurrentContext.CurrentPath;
                //    fileNames = Directory.GetFiles(asmDir, Path.GetFileName(System.Environment.GetCommandLineArgs()[0]));
                //    assemblies = (object[])fileNames;
                //}
                //else
                //{
                asmDir = Context.CurrentContext.CodeBase;
                fileNames = Directory.GetFiles(asmDir, "*.*");
                assemblies = (object[])fileNames;
            }
        ...
        public static void InitializePerfmonCounters()
        {
            ...
            foreach (String categoryName in _catCreationData.Keys)
            {
                ...
                {
                    // skip base creation, this is done by the Altus ctor
        ...
        public void RemoveInstance()
        {
            if (_perfCount != null && (PerformanceCounterCategory.Exists(_perfCount.CategoryName) == false ||
                PerformanceCounterCategory.InstanceExists(_perfCount.InstanceName, _perfCount.CategoryName) == false)) return;
