...
[assembly: Component(ComponentType = typeof(Altus.Core.Serialization.Csv.MessageSerializer))]
namespace Altus.Core.Serialization.Csv
{
    public class MessageSerializer : InitializableComponent, ISerializer<Message>
    {
        ...
        public byte[] Serialize(Message source)
        {
            ...
            //        .Where(s => s.SupportsFormat(StandardFormats.CSV) && s.SupportsType(p.Value.GetType())).FirstOrDefault();
            //    if (serializer == null)
            //    {
            //        // return a serialization error?
            //        throw (new SerializationException("A serializer could not be found for type " + p.Value.GetType().FullName + " support the " + StandardFormats.CSV + " format."));
            //    }
            //    else
            //    {
            ...
            ISerializer serializer = App.Instance.Shell.GetComponents<ISerializer>()
                    .Where(s => s.SupportsFormat(StandardFormats.CSV) && s.SupportsType(source.Payload.GetType())).FirstOrDefault();
            if (serializer == null)
            {
                // return a serialization error?
                throw (new SerializationException("A serializer could not be found for type " + source.Payload.GetType().FullName + " support the " + StandardFormats.CSV + " format."));
            }
            else
            {
