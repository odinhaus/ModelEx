...
[assembly: Component(ComponentType = typeof(Altus.Core.Serialization.Json.MessageSerializer))]
    ...
    public class MessageSerializer : InitializableComponent, ISerializer<Message>
    {
        ...
        {            
            ISerializer serializer = App.Instance.Shell.GetComponents<ISerializer>()
                .Where(s => s.SupportsFormat(StandardFormats.JSON) && s.SupportsType(source.Payload.GetType())).FirstOrDefault();
            if (serializer == null)
            {
                // return a serialization error?
                throw (new SerializationException("A serializer could not be found for type " + source.Payload.GetType().FullName + " support the " + StandardFormats.JSON + " format."));
            }
            else
            {
