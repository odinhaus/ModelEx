...
{
    /// <summary>
    /// This class encodes and decodes JsonParser strings.
    /// Spec. details, see http://www.json.org/
    ///
    /// JsonParser uses Arrays and Objects. These correspond here to the datatypes ArrayList and Hashtable.
    /// All numbers are parsed to doubles.
    /// </summary>
    public class JsonParser
    {
        [ThreadStatic()]
        static Hashtable _instances;
        ...
        /// </summary>
        /// <param name="json">A JsonParser string.</param>
        ...
        /// <summary>
        /// Converts a Hashtable / ArrayList object into a JsonParser string
        /// </summary>
        /// <param name="json">A Hashtable / ArrayList</param>
        /// <returns>A JsonParser encoded string, or null if object 'json' is not serializable</returns>
        ...
        protected static bool SerializeValue(object value, StringBuilder builder)
        {
            try
            {
                ...
                {
                    ArrayList al = new ArrayList((Array)value);
                    success = SerializeValue(al, builder);
                }
                ...
                {
                    builder.Append(((DateTime)value).ToJS());
                }
                ...
                else
                {
                    if (!_instances.ContainsKey(value)
                        ||
                        (
                        _instances.ContainsKey(value)
                        && (int)_instances[value] == _level))
                    {
                        ...
                        {
                            if (mi.GetCustomAttributes(typeof(DataMemberAttribute), true).Length > 0)
                            {
                                if (mi is PropertyInfo)
                                {
                                    if (((PropertyInfo)mi).GetIndexParameters().Length == 0)
                                    {
                                        try
                                        {
                                            table.Add(mi.Name, ((PropertyInfo)mi).GetValue(value, null));
                                        }
                                        catch
                                        {
                                            table.Add(mi.Name, null);
                                        }
                                    }
                                }
                                else if (mi is FieldInfo)
                                {
                                    table.Add(mi.Name, ((FieldInfo)mi).GetValue(value));
                                }
                            }
                        }
                        _instances.Add(value, _level);
                        success = SerializeValue(table, builder);
                    }
                    //success = false;
                }
                return success;
